...
module.exports = {
  issueAccessToken: (payload) => {
    try {
      ...
      logger.error(`Exception at issue Access token [${err.message}]`);
  ...
  issueRefreshToken: (payload) => {
    try {
      ...
      logger.error(`Exception at issue Refresh token [${err.message}]`);
      logger.debug(err.stack());
      /*
          TODO throw http 500 here
          ! Dont send JWT error messages to the client
  ...
  verifyToken: (token) => {
    try {
      ...
      logger.error(`Exception at verify token [${err.message}]`);
      logger.debug(err.stack);
      /*
          TODO throw http 500 here
          ! Dont send JWT error messages to the client
  ...
  verifyAccesToken: (req, res, next) => {
    try {
      ...
      let token = req.body['access_token'] || req.query['access_token'] || parameters['access_token'];

      // when we receive more than 1 token
